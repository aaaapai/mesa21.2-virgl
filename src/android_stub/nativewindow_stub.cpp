...
static void* sHwuiHandle = nullptr;
static native_window_set_surface_damage_t sDamageFuncPtr = nullptr;
static std::once_flag sHwuiInitFlag;
static std::mutex sHwuiLoadMutex;

static void initDamageFunction() {
    std::lock_guard<std::mutex> lock(sHwuiLoadMutex);
    if (sDamageFuncPtr) return;
    ...
    for (int i = 0; libPaths[i] != nullptr; ++i) {
        void* handle = dlopen(libPaths[i], RTLD_LAZY | RTLD_LOCAL);
        if (handle) {
            sHwuiHandle = handle;
            sDamageFuncPtr = (native_window_set_surface_damage_t)dlsym(handle, "native_window_set_surface_damage");
            if (sDamageFuncPtr) {
                ALOGI("Loaded native_window_set_surface_damage from %s", libPaths[i]);
            } else {
                ALOGW("Symbol native_window_set_surface_damage not found in %s", libPaths[i]);
            }
            return;
        }
    }
    ALOGW("Failed to load libhwui.so for damage support");
}

extern "C" {

int native_window_set_surface_damage(ANativeWindow* window,
                                     const void* rects,
                                     size_t num_rects) {
    std::call_once(sHwuiInitFlag, initDamageFunction);
    if (sDamageFuncPtr) {
        return sDamageFuncPtr(window, (const android_native_rect_t*)rects, num_rects);
    }
    return 0;
}
